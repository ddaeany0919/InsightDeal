...
*   **FmkoreaScraper (펨코) [수정: 실시간 급상승 키워드 핫딜 전용 타겟팅]**: 일반 게시판 스크래핑 외에도, 핫딜 페이지(`https://www.fmkorea.com/hotdeal`) 자체를 타겟팅하는 트렌딩 스크래퍼를 분리 운영하여 "핫딜 관련 실시간 급상승 검색어"를 시간 단위로 캐싱, 커뮤니티 및 검색 탭의 추천 키워드로 활용. `&sort_index=pop&order_type=desc` 파라미터를 활용한 '인기/포텐' 필터링 URL 또한 지원.
