...
*   **시간 포매팅 최적화 (`StringUtils.kt`)**: 
    *   `GlobalTimeTicker`: 아이템마다 타이머를 생성하여 메모리가 누수되는 것을 방지하기 위해, 싱글톤 코루틴에서 1분 단위(`delay(60_000L)`)로 `tick`을 발생시킴.
    *   `rememberRelativeTime`: 이 전역 틱을 구독하여 "방금 전", "X분 전", "X시간 전" 등의 텍스트를 리스트 렌더링 시점에만 최소한으로 리컴포지션.
...
*   **AI 구매 가이드 (`AIBuyerGuide`)**: `PriceHistoryPoint` 데이터를 분석하여 "역대 최저가 방어선 갱신! 훌륭한 구매 타이밍입니다." 또는 "현재 가격은 전체 변동의 일정 구간에 위치합니다..." 등의 인사이트 제공.
